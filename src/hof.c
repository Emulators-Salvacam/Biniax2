...
BNX_BOOL hofInit()
{
	...
	{
		strcpy( Hof.arcade[ i ].name, "Ezial                    " );
		Hof.arcade[ i ].score = (cHofEntries - i) * cHofInitScore;

		strcpy( Hof.tactic[ i ].name, "Yoannd26                 " );
...
BNX_BOOL hofEnter( BNX_GAME *game )
{
	...
	do
	{
		...
		if ( inpKeyUp() == BNX_TRUE )
		{
			letterplus();
		}

		if ( inpKeyDown() == BNX_TRUE )
		{
			letterminus();
		}

		if ( inpKeyRight() == BNX_TRUE )
		{
			for (i=0;i<10;i++)
			{
			letterplus();
